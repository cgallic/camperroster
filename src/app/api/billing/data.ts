 ...
 */
/**
 * Distinguishes our claims in stripe_events from the tuition webhook's.
 *
 * Both integrations live on one Stripe account and receive every event. Before
 * the handler column existed, whichever endpoint claimed an id first made the
 * other's claim fail, and that handler treated the failure as a replay and
 * returned 200 — so Stripe never retried an event that was never processed.
 */
const HANDLER = "billing";

export async function claimStripeEvent(eventId: string, type: string): Promise<boolean> {
  const { error } = await supabaseAdmin
    .from("stripe_events")
    .insert({ id: eventId, type, handler: HANDLER });
