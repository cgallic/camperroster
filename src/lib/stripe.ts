...
/** The signing secret for POST /api/stripe/webhook (family tuition). */
...
/**
 * This account now has two Stripe endpoints registered, and Stripe issues a
 * separate signing secret per endpoint. Verifying one endpoint's delivery with
 * the other's secret fails every time, so they are read from different vars.
 *
 *   STRIPE_WEBHOOK_SECRET          -> /api/stripe/webhook   (family tuition)
 *   STRIPE_BILLING_WEBHOOK_SECRET  -> /api/billing/webhook  (camp subscriptions)
 */
export function getBillingWebhookSecret(): string {
  return requireEnv("STRIPE_BILLING_WEBHOOK_SECRET");
}

// ---- Aliases -----------------------------------------------------------------
// The tuition payment code was written against these names. They point at the
// same lazily-built, version-pinned client above; there is no second client.
